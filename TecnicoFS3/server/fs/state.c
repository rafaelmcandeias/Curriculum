...
void inode_table_init() {
    for (int i = 0; i < INODE_TABLE_SIZE; i++) {
        ...
        init_trinco(inode_table[i].trinco);
    }
}
...
// Recebe um inumber e tranco o lock desse inumber
void lock_trinco(int inumber, int flag, int *lock_list, int *last_index){
    lock(inode_table[inumber].trinco, flag);
    lock_list[++*last_index] = inumber;
}


// Recebe um inumber e destanca o trinco
void unlock_trinco(int inumber){
    unlock(inode_table[inumber].trinco);
}


// faz unlock de todos os inodes dentro do array
void remove_locks(int *lock_list, int last_index){
    int counter;
    for (counter = last_index; counter >= 0; counter--){
        unlock_trinco(lock_list[counter]);
    }
}


void singleLock(int inumber, int flag) {
    lock(inode_table[inumber].trinco, flag);
}


void singleUnlock(int inumber) {
    unlock(inode_table[inumber].trinco);
}
